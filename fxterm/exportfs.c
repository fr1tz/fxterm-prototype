...
exslave(void *)
{
	...
	for(;;){
		...
		if(exdebug > 1)
			fprint(2,"exslave dispatch %d %F\n", getpid(), &q->rpc);

		if(waserror()){
			fprint(2,"exslave err %r\n");
			err = up->error;
		}else{
...
Exmkfid(Export *fs, int fid)
{
	...
	if(nf == nil)
		return nil;
	lock(&fs->fidlock);
//	fprint(2,"%d: Exmkfid %d: locked fidlock\n", getpid(), fid);
	h = fid % Nfidhash;
	for(f = fs->fid[h]; f != nil; f = f->next){
		if(f->fid == fid){
			unlock(&fs->fidlock);
//			fprint(2,"%d: Exmkfid %d: unlocked fidlock\n", getpid(), fid);
			free(nf);
			return nil;
		}
	}
...
Exgetfid(Export *fs, int fid)
{
	...
	lock(&fs->fidlock);
//	fprint(2,"%d: Exgetfid %d: locked fidlock\n", getpid(), fid);
	h = fid % Nfidhash;
	for(f = fs->fid[h]; f; f = f->next){
		if(f->fid == fid){
			...
			f->ref++;
			unlock(&fs->fidlock);
//			fprint(2,"%d: Exgetfid %d: unlocked fidlock\n", getpid(), fid);
			return f;
		}
	}
	unlock(&fs->fidlock);
...
{
	lock(&fs->fidlock);
//	fprint(2,"%d: Exputfid %d: locked fidlock\n", getpid(), f->fid);
	f->ref--;
	if(f->ref == 0 && f->attached == 0){
		...
		*f->last = f->next;
		if(f->next != nil)
			f->next->last = f->last;
		unlock(&fs->fidlock);
//		fprint(2,"%d: Exputfid %d: unlocked fidlock\n", getpid(), f->fid);
		free(f);
		return;
	}
	unlock(&fs->fidlock);
