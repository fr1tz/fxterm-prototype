...
typedef struct Colorsub Colorsub;
...
struct DImage
{
	...
	DImage*		next;
	ulong   bcolor; /* color requested in the b (alloc) message */
	int     rice;
	DImage* nextcolor;
	DImage* prevcolor;
};
...
struct Colorsub /* color substitution */
{
	ulong     bcolor;
	ulong     rcolor;
	Colorsub* next;
...
static Memimage *whiterice;
static Memimage *blackrice;

static Rectangle riowindows[1024];
static int       nriowindows;
static int       riowindowsdirty;
...
static DImage* colors;  /* used for ricing: all DImages with a 1x1 rect */
static Colorsub* colorsubs; /* used for ricing: color substitutions */
...
static void
updateriowindows(void)
{
	Memimage *rootimg; /* image allocated on screen that was allocated on screenimage */
	DScreen *rioscreen; /* screen where rio allocates its windows */
	DScreen *ds;
	Memimage *img;
	...
	for(ds = dscreen; ds != nil; ds = ds->next){
		if(ds->dimage->image == screenimage){
			rootimg = ds->screen->rearmost;
			break;
		}
	}
	if(rootimg == nil)
	...
	if(rioscreen == nil)
		return;
	for(img = rioscreen->screen->frontmost; img != nil; img = img->layer->rear){
		riowindows[nriowindows] = img->layer->screenr;
		if(nriowindows+1 == 1024)
			break;
		nriowindows++;
	}
	riowindowsdirty = 1;
...
drawgen(Chan *c, Dirtab*, int, int s, Dir *dp)
{
	...
	/*
	 * Second level contains "info", "new" plus all the clients.
	 */
	switch(t){
	case Q2nd:
		if(s == 0){
	case Qinfo:
			mkqid(&q, Qinfo, 0, QTFILE);
	...
	case Qnew:
		...
		if(s < 2 + sdraw.nclient){
			cl = sdraw.client[s-2];
			if(cl == nil)
				return 0;
			sprint(up->genbuf, "%d", cl->clientid);
			mkqid(&q, ((s-1)<<QSHIFT)|Q3rd, 0, QTDIR);
...
addflush(Rectangle r)
{
	...
	if(screenimage == nil || !rectclip(&r, screenimage->r))
		return;
...
Memimage*
drawinstall(Client *client, int id, Memimage *i, DScreen *dscreen, ulong *bcolor)
{
	DImage *d;
	char buf[9];
	...
	if(bcolor != nil){
		d->bcolor = *bcolor;
		if(i->flags & Frepl){
			if(i->chan==GREY1 && eqrect(i->r, Rect(0,0,1,1)) == 1){
				if(*bcolor == 0xFFFFFFFF){
					//fprint(2, "client %d allocated display->white\n", client->clientid);
					d->rice = Rwhite;
				} else if(*bcolor == 0x000000FF){
					//fprint(2, "client %d allocated display->black\n", client->clientid);
					d->rice = Rblack;
				}
			} else {
				//fprint(2, "client %d allocated new color 0x%08ulx (chan %s)\n",
				//	client->clientid, d->bcolor, chantostr(buf, i->chan));
				d->rice = Rcolor;
				d->nextcolor = colors;
				d->prevcolor = nil;
				if(colors != nil)
...
drawfreedimage(DImage *dimage)
{
	...
	if(dimage->prevcolor != nil)
		dimage->prevcolor->nextcolor = dimage->nextcolor;
	if(dimage->nextcolor != nil)
		dimage->nextcolor->prevcolor = dimage->prevcolor;
	if(dimage == colors)
		colors = dimage->nextcolor;
...
initscreenimage(void)
{
	...
	if(screenimage != nil)
		return 1;
	if(drawscreen == nil){
		fprint(2,"initscreenimage: no draw screen\n");
		return 0;
	}
	screendimage = allocdimage(drawscreen);
	if(screendimage == nil)
		return 0;
	if(!waserror()){
		snprint(screenname, sizeof screenname, "noborder.screen.%d", ++screennameid);
		drawaddname(nil, screendimage, strlen(screenname), screenname);
		poperror();
	}
	screenimage = screendimage->image;
	if(whiterice == nil){
		whiterice = allocmemimage(Rect(0,0,1,1), XRGB32);
		whiterice->flags |= Frepl;
		memfillcolor(whiterice, 0xFFFFFFFF);
	}
	whiterice->clipr = screenimage->r;
	if(blackrice == nil){
		blackrice = allocmemimage(Rect(0,0,1,1), XRGB32);
		memfillcolor(blackrice, 0x000000FF);
		blackrice->flags |= Frepl;
	}
	blackrice->clipr = screenimage->r;
...
drawopen(Chan *c, int omode)
{
	...
	case Qctl:
		...
		if(drawinstall(cl, 0, dn->dimage->image, 0, nil) == 0)
			error(Edrawmem);
...
drawclose(Chan *c)
{
	...
	if(QID(c->qid) < Qcolormap)	/* Qtopdir, Qinfo, Qnew, Q3rd, Q2nd have no client */
		return;
	dlock();
	if(waserror()){
		dunlock();
	...
	while((n-=m) > 0){
		...
		case 'b':
			...
			if(scrnid){
				...
				l->clipr = clipr;
				rectclip(&l->clipr, r);
				if(drawinstall(client, dstid, l, dscrn, nil) == 0){
					memldelete(l);
					error(Edrawmem);
				}
				dscrn->ref++;
			...
			if(!repl)
				rectclip(&i->clipr, r);
			if(drawinstall(client, dstid, i, 0, &value) == 0){
				freememimage(i);
				error(Edrawmem);
			}
			di = drawlookup(client, dstid, 0);
			if(di && di->rice == Rcolor){			
				for(colsub = colorsubs; colsub != nil; colsub = colsub->next){
					if(colsub->bcolor == value){
						//fprint(2, "replacing 0x%08ulx with 0x%08ulx\n", value, colsub->rcolor);				
						value = colsub->rcolor;
						break;
					}	
				}
 ...
 */
		...
		case 'n':
			...
			if(dn == nil)
				error(Enoname);
			if(drawinstall(client, dstid, dn->dimage->image, 0, nil) == 0)
				error(Edrawmem);
			di = drawlookup(client, dstid, 0);
			if(di == 0)
...
//					fprint(2, "o: %R -> %R\n", r, dst->layer->screenr);
		...
		/* write from file: 'z' id[4] R[4*4] nbytes[4] offset[4] filename[x*1] */
		case 'z':
			printmesg(fmt="LRLL", a, 0);
		//	iprint("load %c\n", *a);
			m = 1+4+4*4+4+4;
			fn = (char*)a + m;
			m += strlen(fn) + 1;
			...
			nb = BGLONG(a+21);
			off = BGLONG(a+25);
//			fprint(2, "drawmesg: z: rect %R filename %s nbytes %d offset %d\n", r, fn, nb, off);
			fd = open(fn, OREAD);
			if(fd < 0){
//				fprint(2, "drawmesg: z: open failed: %r\n");
				error("open file failed");
			}
			buf = malloc(nb);
			if(buf == 0){
//				fprint(2, "drawmesg: z: malloc failed: %r\n");
				error(Edrawmem);
			}
			seek(fd, off, 0);
			if(readn(fd, buf, nb) != nb){
//				fprint(2, "drawmesg: z: read failed: %r\n");
				close(fd);
				free(buf);
			...
			close(fd);
			remove(fn);
			y = memload(dst, r, buf, nb, 0);
			free(buf);
			if(y < 0){
//				fprint(2, "drawmesg: z: memload failed: %r\n");
				error("bad writeimage call");
			}
			dstflush(dstid, dst, r);
//			fprint(2, "drawmesg: z: done (%d bytes consumed)\n", m);
			continue;
		}
	}
	poperror();
...
/* For ricing: Called from devrice.c */
void
drawreplacecolor(ulong bcolor, ulong rcolor)
{
	DImage *d;
	Colorsub *cs;

	dlock();
	for(cs = colorsubs; cs != nil; cs = cs->next){
		if(cs->bcolor == bcolor){
			cs->rcolor = rcolor;
			goto update;
		}		
	}
	cs = malloc(sizeof(Colorsub));
	cs->bcolor = bcolor;
	cs->rcolor = rcolor;
	cs->next = colorsubs;
	cs->prev = nil;
	...
	colorsubs = cs;
update:
	for(d = colors; d != nil; d = d->nextcolor){
		if(d->bcolor == bcolor)
			memfillcolor(d->image, rcolor);
	}
	dunlock();
}
...
/* For ricing: Called from devrice.c */
void
drawreplacewhite(ulong rcolor)
{
	dlock();
	memfillcolor(whiterice, rcolor);
	dunlock();
}

/* For ricing: Called from devrice.c */
void
drawreplaceblack(ulong rcolor)
{
	dlock();
	memfillcolor(blackrice, rcolor);
	dunlock();
}

/* For ricing: Called from devrice.c */
int
drawgetbcolors(ulong bcolors[], Rectangle rects[], ulong chans[], ulong flags[], int max)
{
	DImage *d;
	int i, n;

	n = 0;
	memset(bcolors, 0, sizeof(ulong)*max);
	dlock();
	for(d = colors; d != nil; d = d->nextcolor){
		for(i = 0; i < n; i++){
			if(bcolors[i] == d->bcolor)
				break;
		}
		if(bcolors[i] == d->bcolor)
			continue;
		bcolors[n] = d->bcolor;
		rects[n] = d->image->r;
		chans[n] = d->image->chan;
		flags[n] = d->image->flags;
		n++;
	}	
	dunlock();
...
/* For ricing: Called from fxtermdraw.c:/^postfxgenerator */
int
drawgetriowindows(Rectangle windows[], int max, int *changed)
{
	if(max > nriowindows)
		max = nriowindows;
	*changed = riowindowsdirty;
	memcpy(windows, riowindows, nriowindows*sizeof(Rectangle));
	riowindowsdirty = 0;
	return nriowindows;
}
